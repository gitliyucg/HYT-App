...
export class IssueComponent implements OnInit {
    ...
    constructor(public fb: FormBuilder, public http: HttpClient, private router: Router) {
        ...
        http.get(sessionStorage['http'] + '/action/exchanges/GetParams').subscribe(data => {
            ...
            this.formModel.get('Numbers').setValidators([Validators.required, Validators.min(0.01)]);
        });
        ...
        this.formModel.get('Numbers').valueChanges.debounceTime(10).subscribe(val => {
            if (val > 0) {
                ...
                this.rmb = Number(this.formModel.get('Price').value) * this.bizhi;
                this.heji = this.accMul(Math.floor(val), (1 + this.tax));
                if (this.heji > this.HYB){
                    this.maxs = true;
                }else {
                    this.maxs = false;
                }
    ...
    valChange(event){
        // let value = event * this.tax + event;
        // if (value > this.HYB){
        //     this.maxs = true;
        // }else {
        //     this.maxs = false;
